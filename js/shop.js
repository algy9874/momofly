...
import { getFirestore, collection, getDocs, Timestamp} from 'https://www.gstatic.com/firebasejs/9.12.1/firebase-firestore.js';
...
function getOrderRecord(uid){
    ...
    getDocs(colRef).then((snapshot) => {
        snapshot.docs.forEach((doc) => {
            if(uid!=doc.data()["creatorAccountID"]){
                recordList.push({ 
                    ...doc.data(), id: doc.id
                });
            }
        ...
        +'<div class="product__item">'
            ...
            +'<div class="product__item__text">'
                ...
                +'<div class="d-flex">'
                    ...
                    +'<h5>'+numOfApply+'</h5>'
                +'</div>'
                +'<button class="accept-order-btn">提出接取訂單</button>'
            +'</div>'
        +'</div>'
    +'</div>';
    return html;
